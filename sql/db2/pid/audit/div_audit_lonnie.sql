...
DIVAUTH AS (
 SELECT SKU_NBR, WHSE_NBR, DIV_CD, PRIM_WHSE_NBR, IERR, MIN(FLOWCD) as FLOWCD
   FROM RAW
...
SELECT DIVAUTH.SKU_NBR
      ...
      ,CASE DIVAUTH.PRIM_WHSE_NBR
       ...
       ,CHAR(SUBSTR(DIGITS(DECIMAL(RTRIM( 
        ...
        VA1.ART_NBR),14)),2,1) || 
        SUBSTR(DIGITS(DECIMAL(RTRIM( 
        SV1.MSTR_ART_NBR),14)),4,10),13)   AS BAS_UPC_NO 
       ,CASE 
        WHEN DIVAUTH.DIV_CD = '705' 
         AND DIVAUTH.FLOWCD = 'WHP' THEN
          SPACE(4)
        WHEN DIVAUTH.FLOWCD = 'ALC' THEN
          'ALOC'
        ELSE 
          'FMWR' 
        END                                AS FLOW_CD
       ,DIGITS(DECIMAL(FI1.FT_LVL04_CD,3)) AS DEPT
...
SELECT BASE2.CAS_UPC_NO
      ...
      ,COALESCE((SELECT BIL_STU_CD
                   FROM PRD.PID_WHSCA WHS
                  WHERE WHS.CAS_UPC_NO     = BASE2.CAS_UPC_NO
                    AND WHS.SRC_ID         = BASE2.SRC_ID),' ') AS WHS_STU_CD
  FROM BASE2
  WITH UR
;
...
select t1.sku_nbr
      ...
      ,coalesce((select case
                   ...
                   from pdiv pdiv2
                  where pdiv2.cas_upc_no = t1.cas_upc_no
                    and pdiv2.src_id     = t1.src_id
                   limit 1),' ') as whs_sts
      ,case
      ...
      ,t1.ierr
  from mdiv t1
 where t1.div_cd in ('705', '706')
   and t1.src_id in ('791', '797')
--   and 1 = (select 1
--                 from mdiv t2
--                where t2.sku_nbr = t1.sku_nbr
--                  and t2.src_id = '797')
                  
) as results
 where results.div_sts in(' ','D')
--   and results.vld_fr_dt > '2014-01-01'
--   and div_cd = '706'
;
...
select mdiv.sku_nbr
      ...
      ,count(*) as cnt
  from mdiv mdiv
 group by mdiv.sku_nbr, mdiv.div_cd, mdiv.src_id
 having count(*) > 1)
;