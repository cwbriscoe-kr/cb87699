	...
	(select max(pog_qlvl_0.DBDateEffectiveFrom) pog_eff_date_max
	  ...
	  from 
		  ...
		  (select distinct
			  ...
			  from [ckbe3_floorplan_key_15wks_vw] ikbe3_floorplan_key_15wks_vw (NOLOCK)
			  ...
			  where ix_flr_section.dbparentplanogramkey = ix_spc_planogram.dbkey
				and ikbe3_floorplan_key_15wks_vw.DBkey = ix_flr_section.dbparentfloorplankey
		   ) vk
		   -- 
...
FROM (
	SELECT	ISNULL(CAST(ix_spc_product.PartID AS char(18)), ' ') AS SkuNbr
  , coalesce(CAST(SUBSTRING(ikbe3_floorplan_key_15wks_vw.Loc_nbr, 1, 5) AS char(5)),CAST(SUBSTRING(strdata.Loc_Nbr, 1, 5)AS char(5)),' ') AS LocNbr
	...
	,  ix_spc_product.Desc6                             AS FlowCode
 	FROM   CKB.dbo.ix_spc_position AS ix_spc_position WITH (NOLOCK) 
 				 ...
 				 INNER JOIN
 				  [ckbe3_floorplan_key_15wks_vw] AS ikbe3_floorplan_key_15wks_vw WITH (NOLOCK) 
 				  ON ikbe3_floorplan_key_15wks_vw.DBkey = ikbe3_floorsection_keys_vw.dbparentfloorplankey
 	             LEFT OUTER JOIN
 	              (select dbparentfloorplankey,Desc3 as Loc_Nbr
     			  ...
     			  ON ikbe3_floorplan_key_15wks_vw.DBkey = strdata.dbparentfloorplankey
	WHERE  ix_spc_planogram.DBStatus IN (1, 2, 5) 
	AND  CAST(ix_spc_product.Desc6 AS varchar(3)) IN ('DTS', 'RMA')  
	AND ix_spc_product.Value14 IN (20, 30) 
  AND ix_spc_product.Desc45  = '87'                                                                                                                     
  AND ix_spc_product.PartID  > ' '                                                            
  AND coalesce(CAST(SUBSTRING(ikbe3_floorplan_key_15wks_vw.Loc_nbr, 1, 5) AS char(5))
